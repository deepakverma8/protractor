...
Protractor.prototype.waitForAngular = function() {
  ...
  this.driver.executeAsyncScript(clientSideScripts.testForAngular, 10, false).
      then(function(angularTestResult) {
        if (!angularTestResult[0]) {
          var message = angularTestResult[1];
          throw new Error('Angular could not be found on the page ' +
              'TODOdestination' + ' : ' + message);
        }
      }, function(err) {
...
Protractor.prototype.doBootstrap = function(destination, opt_timeout) {
  ...
  // Make sure the page is an Angular page.
  self.driver.executeAsyncScript(clientSideScripts.testForAngular, timeout, true).
      then(function(angularTestResult) {
        if (!angularTestResult[0]) {
          var message = angularTestResult[1];
          throw new Error('Angular could not be found on the page ' +
  ...
  for (var i = 0; i < this.mockModules_.length; ++i) {
    ...
    if (isFunction(mockModule.script)) {
